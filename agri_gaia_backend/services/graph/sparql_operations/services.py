...
def get_default_graph(minio_server, bucket, service_name, service_id):
    ...
    id_string = "https://" + minio_server + "/" + \
        bucket + "/services/" + str(service_id)
    accessURL = URIRef("https://" + minio_server + "/" + \
        bucket + "/services/definitions/" + str(service_name))
...
def create_graph(minio_server: str, bucket: str, service_name: str, service_id: int, description: str, url: str, labels):
...
def create_graph_autogenerated(file_json, minio_server, service_id, bucket):
    id_string = "https://" + minio_server + "/" + \
        bucket + "/services/" + str(service_id) + "Service_Autogenerated"
    ...
    g.add((api_service, RDF.type, URIRef('http://w3id.org/agri-gaia-x/asset#AgriApiDescription')))
    if "title" in file_json['info']:
        g.add((api_service, DCTERMS.title, Literal(file_json['info']['title'])))
    if "name" in file_json['info']['contact']:
        g.add((api_service, URIRef('http://w3id.org/gaia-x/gax-trust-framework#name'), Literal(file_json['info']['contact']['name'])))
    if "description" in file_json['info']:
        g.add((api_service, DCTERMS.description, Literal(file_json['info']['description'])))
    ...
    if "url" in file_json['servers'][0]:
        ...
        g.add((api_service, URIRef('http://www.w3.org/ns/hydra/core#entrypoint'), Literal(entrypoint)))
    if "version" in file_json['info']:
        g.add((api_service, DCAT.version, Literal(file_json['info']['version'], datatype=XSD.string)))
    for path in file_json['paths']:
        ...
        if path.find('{') == -1:
            ...
            #print(path)
            g.add((api_service, URIRef('http://w3id.org/agri-gaia-x/asset#availablePath'), URIRef(pathURI)))
            g.add((URIRef(pathURI), RDF.type, URIRef('http://w3id.org/agri-gaia-x/asset#ApiPath')))
            g.add((URIRef(pathURI), URIRef('http://www.w3.org/ns/hydra/core#template'), Literal(path, datatype=XSD.string))) 
        else:
            ...
            g.add((URIRef(pathURI), URIRef('http://w3id.org/agri-gaia-x/asset#queryParameterMapping'), URIRef(param_map)))
            g.add((URIRef(param_map), RDF.type, URIRef('http://www.w3.org/ns/hydra/core#IriTemplate')))
        ...
        for path_op in file_json['paths'][path]:
            ...
            status_list = []
            #print(path_op)
            g.add((URIRef(pathURI), URIRef('http://www.w3.org/ns/hydra/core#supportedOperation'), bnode_sup_op))
            g.add((bnode_sup_op, RDF.type, URIRef('http://www.w3.org/ns/hydra/core#Operation')))
            g.add((bnode_sup_op, URIRef('http://www.w3.org/ns/hydra/core#method'),  Literal(path_op, datatype=XSD.string)))
            ...
            for status_resp in file_json['paths'][path][path_op]['responses']:
                ...
                if 'content' in file_json['paths'][path][path_op]['responses'][status_resp]:
                    bnode_resp = BNode()
                    ...
                    g.add((bnode_sup_op, URIRef('http://www.w3.org/ns/hydra/core#returns'), bnode_resp))
                    g.add((bnode_resp, RDF.type, URIRef('http://w3id.org/agri-gaia-x/asset#responseObject')))
                    for resp_obj in file_json['paths'][path][path_op]['responses'][status_resp]['content']:
                        #g.add((bnode_resp, URIRef('http://w3id.org/agri-gaia-x/asset#responseObjectType'), Literal(resp_obj, datatype=XSD.string)))
                        g.add((bnode_sup_op, URIRef('http://w3id.org/agri-gaia-x/asset#responseSerialization'), Literal(resp_obj, datatype=XSD.string)))

                        resp_type = file_json['paths'][path][path_op]['responses'][status_resp]['content'][resp_obj]['schema']
                        ...
                        g.add((bnode_resp, URIRef('http://www.w3.org/ns/hydra/core#supportedProperty'), bnode_resp_obj))
                        g.add((bnode_resp_obj, RDF.type, URIRef('http://w3id.org/agri-gaia-x/asset#responseObject')))
                        ...
                        if '$ref' in resp_type_schema:
                            ...
                            resp_ref = resp_type_schema['$ref'][resp_type_schema['$ref'].rfind('/')+1:]
                            resp_schema = file_json['components']['schemas'][resp_ref]['properties']
                            for prop in resp_schema:
                                ...
                                g.add((bnode_resp_obj, URIRef('http://www.w3.org/ns/hydra/core#supportedProperty'), bnode_sup_prop))
                                g.add((bnode_sup_prop, RDF.type, URIRef('http://www.w3.org/ns/hydra/core#SupportedProperty')))
                                g.add((bnode_sup_prop, URIRef('http://www.w3.org/ns/hydra/core#title'), Literal(prop, datatype=XSD.string)))
                                if 'type' in resp_schema[prop]:
                                    #print(resp_schema[prop]['type'])
                                    g.add((bnode_sup_prop, DCTERMS.type, Literal(resp_schema[prop]['type'], datatype=XSD.string)))
                        elif 'format' in resp_type_schema:
                            print(resp_type_schema['format'])
                            g.add((bnode_resp_obj, URIRef('http://www.w3.org/ns/hydra/core#supportedProperty'), Literal(resp_type_schema, datatype=XSD.string)))


            #print(status_list)
            for status in status_list:
                g.add((bnode_sup_op, URIRef('http://www.w3.org/ns/hydra/core#possibleStatus'),  Literal(status, datatype=XSD.int)))
    ...
    g.add((api_service, DCTERMS.issued, Literal(datetime.today().strftime('%Y-%m-%d %H:%M:%S'), datatype=XSD.dateTime)))
    g.add((api_service, DCTERMS.modified, Literal(datetime.today().strftime('%Y-%m-%d %H:%M:%S'), datatype=XSD.dateTime)))
