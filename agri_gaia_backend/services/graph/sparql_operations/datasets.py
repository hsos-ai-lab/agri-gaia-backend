...
def get_default_graph(minio_server, bucket, dataset_name, dataset_id):
    ...
    id_string = "https://" + minio_server + "/" + \
        bucket + "/datasets/" + str(dataset_id)
...
def create_graph(label_uris, location_uris, minio_server: str, bucket: str, dataset_name: str, dataset_id: int, description: str, metadata: str, dataset_type: str, config_files, annotation_labels):
    """
    ...
    graph = _handle_type(graph=graph, metadata=metadata, dataset_id=datasetid, dataset_type=dataset_type, config_files=config_files)
...
def _handle_type(graph, metadata, dataset_id: URIRef, dataset_type: str, config_files):
    ...
    if dataset_type is "AgriSyntheticImageDataResource":
        graph, metadata = _handle_synthetic_dataset(graph=graph, metadata=metadata, config_files=config_files, fuseki_id=dataset_id)
...
def _handle_synthetic_dataset(graph, metadata, config_files, fuseki_id):
    if(config_files["config"] is not None and config_files["asset_catalog"] is not None):
        ...
        asset_catalog_file_yaml = yaml.safe_load(config_files["asset_catalog"].file.read()) 
        ...
        if config_file_yaml['steps']:
            graph.add((fuseki_id, URIRef('http://w3id.org/agri-gaia-x/asset#imageCount'), Literal(config_file_yaml['steps'])))

        if config_file_yaml['postprocessing']['Postprocessing/BoundingBoxes'][0]['id']:
            graph.add((fuseki_id, URIRef('http://w3id.org/agri-gaia-x/asset#boundingBoxFormat'), Literal(config_file_yaml['postprocessing']['Postprocessing/BoundingBoxes'][0]['id'], datatype=XSD.string)))

        base_camera_plugins = config_file_yaml['sensor']['Base Plugins/Camera'][0]
        if base_camera_plugins['name']:
            bnode_device = BNode()
            graph.add((fuseki_id, URIRef('http://w3id.org/agri-gaia-x/asset#deviceUsed'), bnode_device))
            graph.add((bnode_device, RDF.type, URIRef('http://w3id.org/agri-gaia-x/asset#VirtualCamera')))
            
            if base_camera_plugins['sensor_width']:
                graph.add((bnode_device, URIRef('http://w3id.org/agri-gaia-x/asset#cameraSensor'), Literal(str(base_camera_plugins['sensor_width'])+' mm', datatype=XSD.string)))
            
            if base_camera_plugins['focal_length']:
                graph.add((bnode_device, URIRef('http://w3id.org/agri-gaia-x/asset#cameraLense'), Literal('Focal length '+str(base_camera_plugins['focal_length'])+' mm', datatype=XSD.string)))

            if base_camera_plugins['motion_blur']['rolling_shutter']['enabled']:
                graph.add((bnode_device, URIRef('http://w3id.org/agri-gaia-x/asset#cameraShutter'), Literal('Rolling shutter enabled', datatype=XSD.string)))

            if base_camera_plugins['resolution']:
                graph.add((bnode_device, URIRef('http://w3id.org/agri-gaia-x/asset#cameraImageWidth'), Literal(base_camera_plugins['resolution'][0])))
                graph.add((bnode_device, URIRef('http://w3id.org/agri-gaia-x/asset#cameraImageHeight'), Literal(base_camera_plugins['resolution'][1])))
                graph.add((fuseki_id, URIRef('http://w3id.org/agri-gaia-x/asset#avgImageWidth'), Literal(base_camera_plugins['resolution'][0])))
                graph.add((fuseki_id, URIRef('http://w3id.org/agri-gaia-x/asset#avgImageHeight'), Literal(base_camera_plugins['resolution'][1])))

            if base_camera_plugins['shutter_speed']:
                graph.add((fuseki_id, URIRef('http://w3id.org/agri-gaia-x/asset#imageExposure'), Literal('Shutter speed '+str(base_camera_plugins['shutter_speed'])+' sec',datatype=XSD.string)))

            if base_camera_plugins['depth_of_field']['aperture']:
                img_exposure = 'Aperature f/'+str(base_camera_plugins['depth_of_field']['aperture'])
                if base_camera_plugins['depth_of_field']['autofocus']:
                    img_exposure = img_exposure+', autofocus enabled'
                graph.add((fuseki_id, URIRef('http://w3id.org/agri-gaia-x/asset#imageExposure'), Literal(img_exposure, datatype=XSD.string)))

            if base_camera_plugins['outputs']['Base Plugins/RGB']:
                graph.add((fuseki_id, URIRef('http://w3id.org/agri-gaia-x/asset#imageColorScheme'), Literal('RGB', datatype=XSD.string)))

            if base_camera_plugins['outputs']['Base Plugins/PixelAnnotation']:
                for item in base_camera_plugins['outputs']['Base Plugins/PixelAnnotation'][0]:
                    graph.add((fuseki_id, URIRef('http://w3id.org/agri-gaia-x/asset#availablePixelAnnotation'), Literal(item, datatype=XSD.string)))
                ...
                for base_plugin_item in scene_item:
                    if base_plugin_item in ('texture', 'models'):
                        if(type(scene_item[base_plugin_item]) is list):
                            asset_index.append(str(scene_item[base_plugin_item][0]).split('/'))
                    ...
                    if base_plugin_item == 'environment_image':
                        if 'random_selection' in scene_item['environment_image']:
                            asset_index.append(scene_item['environment_image']['random_selection'][0].split('/'))
        ...
        for item in asset_index:
            ...
            if 'agrovoc_uri' in asset_catalog_file_yaml[item[0]]['assets'][item[1]]:
                graph.add((fuseki_id, DCTERMS.subject, URIRef(asset_catalog_file_yaml[item[0]]['assets'][item[1]]['agrovoc_uri'])))
                graph.add((fuseki_id, URIRef('http://w3id.org/agri-gaia-x/asset#objectsInTheScene'), URIRef(asset_catalog_file_yaml[item[0]]['assets'][item[1]]['agrovoc_uri'])))
